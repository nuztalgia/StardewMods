...
internal static class ContentPatcherBridge {
    ...
    if ((ManagedConfigs == null) && (AllRawConfigs != null)) {
      ManagedConfigs =
          AllRawConfigs
              ...
              .ToImmutableDictionary(kvp => kvp.Key, kvp => ParseRawModConfig(kvp.Key, kvp.Value!));
      AllRawConfigs = null;
      OnContentPatcherInitialized?.Invoke();
    }
  }

  private static IEnumerable<ConfigFieldData> ParseRawModConfig(
      string contentPackId, object rawConfigDict) {
    ...
    if (TryInvokeMethod(rawConfigDict, "get_Keys") is IEnumerable<string> configKeys) {
      foreach (string configKey in configKeys) {
        ...
        if ((TryInvokeMethod(rawConfigDict, "TryGetValue", args) is true)
          ...
          configData.Add(ParseRawConfigField(configKey, rawConfigField));
        }
      }
    }
    return configData;
  ...
  private static ConfigFieldData ParseRawConfigField(string configKey, object rawConfigField) {
